...
#define STEP_SIZE 1.0 // Step size for the grid
...
// Function to perform one iteration of the Jacobi method for Poisson's equation
...
// Function to solve Poisson's equation using the Jacobi method
void solvePoisson(double grid[N][N]) {
    double newGrid[N][N];
    int iter = 0;
...
int main() {
    ...
    printf("Grid values after solving Poisson's equation:\n");
    printGrid(grid);
